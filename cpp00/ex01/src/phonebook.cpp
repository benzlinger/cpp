...
{
	this->_contacts[this->_index].set_first_name(get_info("Enter first name: "));
	this->_contacts[this->_index].set_last_name(get_info("Enter last name: "));
	this->_contacts[this->_index].set_nickname(get_info("Enter nickname: "));
	this->_contacts[this->_index].set_phone_number(get_info("Enter phone number: "));
	this->_contacts[this->_index].set_secret(get_info("Enter darkest secret: "));
	this->_index++;
	if (this->_index == 8)
		this->_index = 0;
...
void	PhoneBook::show_contact_info(int index)
{
	if (index > 0 && index <= this->_amount_contacts)
	{
		...
		std::cout << "First name:     " << this->_contacts[index - 1].get_first_name() << std::endl;
		std::cout << "Last name:      " << this->_contacts[index - 1].get_last_name() << std::endl;
		std::cout << "Nickname:       " << this->_contacts[index - 1].get_nickname() << std::endl;
		std::cout << "Phone number :  " << this->_contacts[index - 1].get_phone_number() << std::endl;
		std::cout << "Darkest secret: " << this->_contacts[index - 1].get_secret() << std::endl;
	}
	else
...
void	PhoneBook::show_contacts(void)
{
	...
	else
	{
		...
		for (int i = 0; i < this->_amount_contacts; i++)
		{
			...
			print_info(this->_contacts[i].get_first_name());
			print_info(this->_contacts[i].get_last_name());
